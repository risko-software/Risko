...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    createNode: function (data) {
      ...
      var $nodeDiv = $('<div onclick="caja_accion(\''+data.mi_tipo+'\', \''+data.mi_id+'\');" ' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + '>')
        .addClass('node ' + (data.className || '') +  (level > opts.visibleLevel ? ' slide-up' : ''));
      if (opts.nodeTemplate) {
        $nodeDiv.append(opts.nodeTemplate(data));
      } else {
        $nodeDiv.append('<div class="title">' + data[opts.nodeTitle] + '</div>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content_caja" >' + (data[opts.nodeContent] || '') + '</div>' : '');
      }
      //
